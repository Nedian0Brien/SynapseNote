...
- 상태: Active, v2 writer/default implementation connected (blank/template creation now targets v2;
  existing-folder onboarding remains an explicit v1 compatibility path). Core/read contract,
  server writer/API, durable migration journal/task gate, app cell/row adapter, title/move writer,
  native lifecycle metadata/writer, capability response, semantic revisions, identity/copy/move
  safeguards, canonical/computed export, migration owner/dependency/derived preflight,
  retention-aware recovery inspection, parser process-timeout, generic writer crash recovery,
  all-surface derived parity, lifecycle performance budgets, supported-max reliability soak,
  v1→v2→undo round-trip evidence, multi-database migration UX, and V1 edit interception are
  implemented. Desktop parity is covered by focused fixtures; deferred-cleanup operator sign-off
  and external rollout approval remain release-operations gates. The compatibility writer remains
  available only behind an explicit import/migration/test seam.)
- 최종 수정: 2026-07-27
...
- Core manifest는 v1 legacy/read compatibility와 v2 owner-table writer를 함께 검증하고,
  source별 `markdown_table` storage binding과 migration alias metadata를 strict schema로
  검사한다. 새 blank/template database는 v2를 기본으로 생성하고 existing-folder onboarding은
  v1 compatibility source로 남긴다.
...
- V2 plan은 caller가 임의의 `rec_*` ID만 지정하고 `documentId`를 생략한 신규 row를
  `record_identity_required` blocker로 거부한다. 기존 row update는 index의 stable
  document identity를 재사용하고, 새 row만 explicit document identity를 요구한다.
...
- App에는 `/api/databases/markdown-table/mutate` 전용 client/gateway와 owner revision을
  ...
  conflict/refresh semantics와 함께 정리한다. Row create는 owner가 허용한 content-root 상대
  `.md` 경로의 normal Markdown document를 만들며, existing title/body template과 Unique ID
  allocation은 별도 product gate다. Renderer-level receipt-backed UI undo/redo는 아직 gated다.
...
- v1 reader/export/migration writer는 compatibility scope에 남고, production server의
  Data Plane/automation/common commit engine은 `storage_read_only` 또는
  `migration_required`로 product mutation을 차단한다. `allowLegacyV1Mutation`은
  테스트/import 경계에만 남는 명시적 seam이며 새 v2 creation path는 owner-table writer를
  사용한다. 반복 template와 durable task 같은 내부 direct-commit caller도 common engine
  guard를 통과해야 한다.

현재 branch에서 코드로 구현했지만 별도 release record 없이는 public rollout으로 간주하지 않는
범위는 다음과 같다.

- v2 owner-table mutation은 dedicated storage-aware API/MCP/app route와 generic plan/commit
  adapter 모두에서 활성화되며, cell과 row create/delete/title/lifecycle가 같은 writer를
  사용한다. 새 blank/template/inline database와 일반 row 생성은 v2를 기본으로 선택하고,
  existing-folder binding만 명시적 v1 compatibility choice를 유지한다. Renderer-level
  receipt-backed UI undo/redo와 public rollout approval은 release gate다.
- Durable v1→v2 migration task의 journal/gate와 cold verification은 연결되어 있다. migration
...
- Formula/Rollup은 owner table에 저장하지 않도록 schema가 보장하고, stable-ID compile/
  ...
  snapshot comparator와 server cold verification도 연결됐다. Query/API/MCP/export가 같은
  derived revision을 운반하고 migration freeze Formula/Rollup value/error deep comparison과
  round-trip fixture가 통과한다. Lifecycle benchmark와 supported-max soak는 release evidence에
  고정됐다. Desktop parity focused fixture와 cross-surface v1 guard fixture가 통과했고,
  cross-editor production soak는 release 이후 운영 관찰 항목이다.
...
- [x] **V2-A-003 — Title/document contract.** 기존 단일 Title property ID를 첫
...
- [x] **V2-A-007 — Revision contract.** Manifest, owner document, table structure,
...
공용 유틸과 새 blank/template database의 v2 default routing은 구현됐지만, public new-default
release approval(실제 pilot·operator sign-off·rollback window)은 여전히 M5 gate다.
...
- [x] **V2-B-009 — Parser security and fuzzing.** Bounded parser와 fuzz corpus를
...
`markdown-table-fuzz.test.ts`에 malformed marker/table, owner-size, cell-size, invalid
UTF-8, 100,001-row escape corpus를 등록했다. `bun run test:file --
packages/core/src/database/markdown-table-fuzz.test.ts`는 5 pass / 773 assertions이며,
child Bun process에는 4초 timeout, SIGKILL, 4 KiB output cap을 적용해 hang/OOM 경계를
검증한다.
...
- [x] **V2-C-004 — Copy/paste identity.** Owner block 또는 owner document 복사 시
...
- [x] **V2-C-007 — Membership/delete semantics.** Row removal, document deletion,
...
- [x] **V2-C-008 — Identity repair.** Missing/duplicate document ID, duplicate row,
  stale alias를 read-only diagnose하고 repair plan을 생성한다. 완료 기준: repair가 stable
  reference rewrite scope와 blocker/impact(잠재적 loss 포함)를 보여주며 approval/undo를 거친다.

완료 증거: `packages/core/src/database/document-identity.ts`,
...
명시하지 않으면 실패함을 확인한다. `cloneDatabaseMarkdownOwnerIdentity`는 owner marker만
새 database/source/block identity로 바꾸고 table/prose bytes를 보존하며 duplicate marker를
거부한다. `database-markdown-table-writer-process-crash.test.ts`와 production `copy_row`
route는 linked-view copy를 `reference_only`로 거부하고 명시적 document identity를 가진
duplicate-document copy를 atomic undo와 함께 검증한다. `markdown-table-link-rewrite.ts`와
writer move test는 owner-table 내부의 정확한 document/relation target rewrite와 ID 보존을
확인한다. `markdown-table-identity-repair.ts`는 missing/invalid/duplicate document ID,
duplicate row, broken link, stale alias를 content-free plan으로 진단하고 제안된 ID와
exact before/after hash가 있는 Markdown rewrite action을 만든다. `database-repair.test.ts`
는 approval token, durable undo token, 재시작 후 exact byte undo, intervening edit 차단을
검증하며 HTTP/MCP/진단 UI가 같은 plan/undo 계약을 사용한다.
`database-plan.test.ts`의 explicit-v2-ID fixture는 `documentId` 없는 신규 row를
`record_identity_required`로 막고 기존 row update만 허용한다.
...
- [x] **V2-D-003 — Title edit.** Table Title edit와 document H1/title edit가 같은
...
- [x] **V2-D-005 — Multi-file journal.** Manifest, owner, linked documents 변경의
...
- [x] **V2-D-006 — Manifest activation boundary.** Activation 전 write는 v1을 계속
  ...
  materialize하지 않는다.
- [x] **V2-D-007 — Verified commit.** Commit 후 cold parse/index와 intended logical
...
Native server archive/audit/layout mutation과 title/move/lifecycle route는 구현되어 있다.
`database-migration-process-crash.test.ts`가 독립 child process의 staging/activation
every-file SIGKILL과 journal recovery를 증명하고, differential/verification fixtures가
cold logical diff를 증명한다. `database-markdown-table-writer-process-crash.test.ts`는
title의 linked-document/owner 두 write 경계에서 SIGKILL을 주입해 mixed bytes를
`recovery_required`, all-after bytes를 `committed`로 분류한다. 일반 mutation의 disk-full/
permission-loss matrix와 user-facing receipt-backed redo/full renderer parity는 여전히
D 영역의 별도 product gate다.
...
- [x] **V2-E-005 — Snapshot query parity.** Filter/sort/group/projection/pagination이
...
- [x] **V2-E-006 — Search deduplication.** Generic document body/title와 database cell
...
- [x] **V2-E-008 — Export semantics.** Canonical Markdown export와 computed snapshot
...
검증한다. `database-v1-v2-differential.test.ts`는 canonical ID alias를 정규화한 v1/v2
logical record, filter/sort/select/aggregate/page cursor와 search provenance를 비교하고,
...
- [x] **V2-F-007 — Derived revision.** Manifest/table/dependency/permission/evaluation
...
- [x] **V2-F-009 — Migration equivalence.** Frozen v1 input과 v2 staged input으로
...
core/server/browser에서 통과시킨다. `database-data-plane.test.ts`,
`database-migration-roundtrip.test.ts`, `mcp/tools/database.test.ts`는 query/API/MCP/export의
derived revision equality와 frozen v1→v2 Formula/Rollup value comparison을 검증한다.
...
- [x] **V2-G-004 — Yjs/ProseMirror mapping.** Database UI mutation을 owner document의
...
- [x] **V2-G-007 — Attribution/history.** Cell/document/schema transaction의 actor와
...
`database-index-git-sync.test.ts`는 branch checkout 뒤 v2 manifest/owner/linked-document
index를 cold rebuild하고 stale branch state를 제거하는 경계를 검증한다. CLI owner merge와
manifest rebase/remote round-trip, migration partial-journal recovery까지 합친 fixture로
G-006 기준을 닫았다. `markdown-table-crdt.ts`는 ProseMirror transaction metadata를
stable `(ownerBlockId, recordId, propertyId)` mutation으로 변환하고 실제 두 Yjs client의
different-cell convergence 및 same-cell/delete-vs-edit conflict를 분류한다.
`database-markdown-table-writer.ts`의 receipt와 durable journal은 actor, logical identity,
before/after revision을 함께 기록하며 `database-record-index.test.ts`가 이를 검증한다.
...
- [x] **V2-H-005 — Title and document plan.** Document ID assignment, v1/document title
...
- [x] **V2-H-007 — Relation/derived preflight.** Relation target mapping과 frozen
...
- [x] **V2-H-010 — Migration fixture matrix.** Generated blank, existing folder,
...
`markdown-table-migration.test.ts`는 title conflict에 대해 `keep_document_title`,
`use_record_title`, `custom_title`의 document title·first wikilink alias 동시 반영과
invalid custom title blocker를 검증하고, server task/API/MCP schema가 같은 choice를
plan input/hash에 전달한다. `derivedBaselines`는 UTC evaluation timestamp, timezone,
locale, permission revision을 strict parse/freeze하여 database별 plan hash에 묶는다.
`database-migration-roundtrip.test.ts`는 cross-source relation target mapping과 Formula/
Rollup value를 frozen baseline으로 preview/apply/cold verify/undo하고, missing baseline을
`derived_baseline_required` blocker로 확인한다. `markdown-table-migration-matrix.test.ts`는
generated blank, existing folder, inline/full-page owner, all stored scalar/link property
types, invalid raw/select, lifecycle metadata, CRLF/BOM/Unicode, and size-boundary fixtures를
각각 documented ready/blocker outcome과 함께 실행한다. Layout/alias preservation is also
covered by the migration and record materialization fixtures.
...
- [x] **V2-I-004 — Isolated staging.** v2 manifest/owner/documents를 task-scoped same-volume
...
- [x] **V2-I-006 — Manifest activation cutover.** V1-readable additive artifact를
...
- [x] **V2-I-007 — Post-commit cold rebuild.** Cache/index 삭제 후 v2 catalog/query를
...
- [x] **V2-I-010 — Cancel/retry semantics.** Commit 전 cancel과 commit 후 rollback을
  ...
  재사용되지 않는다.
- [x] **V2-I-011 — Deferred cleanup.** V1 generated artifacts와 migration compatibility
...
- [x] **V2-I-012 — Recovery tooling.** Interrupted task inspect, resume, rollback,
...
evidence다. `database-migration-process-crash.test.ts`는 독립 child process에 SIGKILL을
...
inspect/preview-cleanup/cleanup`이 recovery surface를 노출한다. Cleanup은 retention-expired
content-free plan hash와 `approve:<hash>`가 없으면 실행되지 않으며, task material만 지우고
linked documents는 보존한다. `database-recovery-runbook.test.ts`는 실제 임시 Git seed를
clone한 독립 workspace에서 inspect→rollback→cleanup을 실행해 L-007 runbook을 재현한다.
...
- [x] **V2-J-004 — Linked view reference-only behavior.** Linked view가 owner source를
...
- [x] **V2-J-005 — Migration preview UX.** Source/record/property/path diff, blockers,
  ...
  `DatabaseMigrationRecoveryPanel`은 여러 database ID를 선택해 정확한 preview scope/hash를
  만들고, nested owner/title choice와 details 기반 diff를 keyboard-accessible control로
  제공한다. `MIGRATION-UX-001`은 8 pass / 28 assertions로 selection, bounded diff, loss
  acknowledgement, persisted task reconnect, retry/rollback을 검증한다.
- [x] **V2-J-006 — Progress/recovery UX.** Durable task phase, progress, cancelability,
  retry, rollback, recovery-required를 표시한다. 완료 기준: app restart 후 같은 task를 재연결하고
  stale optimistic state를 success로 표시하지 않는다.
- [x] **V2-J-007 — V1 edit interception.** V1 source edit에서 silent auto-migration 대신
  read-only 설명과 migration CTA를 제공한다. 완료 기준: table/page/API/MCP/automation 모두 같은
  policy를 따르고 read/export는 계속 가능하다. App workspace/inline status panel, plan
  conflict extension, MCP commit problem mapping, and automation run terminal state all use
  the same non-retryable `start_migration` recovery action; read/export paths remain available.
  Focused evidence is `V1-GUARD-UI-001`, `V1-GUARD-API-MCP-001`, and
  `V1-GUARD-AUTOMATION-001`.
- [x] **V2-J-008 — CLI/headless flow.** Inventory/plan/apply/status/inspect/resume/rollback/cleanup을
  machine-readable JSON과 human summary로 제공한다. 완료 기준: explicit approval token/plan hash
  없이는 apply하지 않고, retention 전 cleanup은 거부하며, CI fixture migration을 UI 없이 실행할 수 있다.
...
recovery action, CLI `database migration inspect/cleanup` commands, app offline stable-ID rebase가
추가됐다. Server title/move/lifecycle route와
capability response는 구현됐고, writer `copy_row`는 `linked_view`를 `reference_only`로
차단하며 row delete는 linked document를 보존하고 missing source는 `document_not_found`로
...
manifest/owner/linked-document preview를 50개 단위로 제한하고 blocker, explicit approval,
non-lossless acknowledgement, durable progress/cancel/resume state를 접근 가능한 status/alert로
노출한다. `DatabaseMigrationRecoveryPanel`은 v1 writer guard를 받은 workspace status surface와
inline surface에 연결되어 preview의 exact plan hash/timestamp로 task를 시작하고 durable task를
polling한다. Task ID를 database scope별 local storage에 보존해 remount 후 inspect하고, retry와
rollback은 exact task revision으로 호출한다. Content-free blocker에서 title/owner path choice를
keyboard-selectable control로 제공한다. 다중 데이터베이스도 선택된 ID 집합의 preview hash와
task scope로 묶어 승인한다. Plan conflict의 `source_record_migration_required`도 같은
migration CTA로 분류된다.

## K. Performance, scalability, security, and reliability gates

- [x] **V2-K-001 — Reference benchmark corpus.** 100-row, 1k-row, supported-max,
...
- [x] **V2-K-002 — Numeric hard limits.** Benchmark로 owner bytes/rows/columns/cell/
  relation target limits를 확정한다. 완료 기준: 실제 숫자가 core constants, API capability,
  UI guidance, docs에 동일하고 max+1 plan이 write 전에 거부된다.
- [x] **V2-K-003 — Latency/memory budgets.** Cold parse/index, warm open, cell commit,
  ...
  policy가 문서화된다.
- [x] **V2-K-004 — Bounded rendering/query.** Viewport DOM, query, export, context pack이
...
- [x] **V2-K-007 — Permission noninterference.** Relation/Rollup/query/search/export/migration
...
- [x] **V2-K-008 — Telemetry privacy.** Migration/performance metrics에서 path/title/cell/
  body/expression/person data를 제외한다. 완료 기준: payload schema allowlist test와 redacted local
  diagnostics snapshot이 통과한다.
- [x] **V2-K-009 — Reliability soak.** Repeated edit/reload/Git sync/migration/rollback을
...
ENOSPC/EACCES failure injection, symlink/path guards, stale lock recovery가 K-005 기준을
닫는다. Case-collision/outside-root guard와 전체 migration matrix도 해당 fixture에 포함된다.
또한 lifecycle benchmark가 cold/index/incremental/Formula/Rollup/context뿐 아니라 cell-commit와
migration-throughput p95 및 process-relative RSS budget을 기록하고 deterministic baseline 대비
regression policy를 적용하므로 K-003을 닫는다. `database-v2-reliability-soak.test.ts`는
50,000행 owner와 5,000개 linked Markdown document를 독립 Git workspace와 bare remote에 만들고
10회 edit→commit/push/fetch→branch checkout→migration rollback→cleanup을 반복한다. 홀수 회차마다
새 store/parser로 cold reload하고 50,000행을 재확인하며, peak heap delta 768MiB, inflight journal
0건, commit lock 잔존 0건, 마지막 owner value를 검증하므로 K-009를 닫는다. Test는 의도적으로
release gate에서만 실행하는 180초 bounded soak이며, cross-editor production soak와 process-wide
OOM ceiling은 별도 운영 관찰 대상이다.
query/export/context pack은 500-row/500-record bound와 50k resource fixture가 있지만
supported-max 50k DOM virtualization, query cancellation, and resource bounds가 K-004
기준을 닫는다. 상세 실행 결과는
[`0008-v2-release-evidence.md`](./0008-v2-release-evidence.md)에 고정한다.

## L. Test matrix, documentation, rollout, and v1 removal

- [x] **V2-L-001 — Core conformance suite.** Manifest/marker/codec/identity/revision/
  formula vectors를 browser/Node에서 공유한다. 완료 기준: 같은 fixture가 동일 typed/error result와
  documented canonical bytes를 생성한다.
- [x] **V2-L-002 — Differential v1/v2 suite.** 같은 logical corpus를 두 reader로
...
- [x] **V2-L-003 — Migration round-trip suite.** Fixture마다 v1→v2 apply, cold rebuild,
  v2 mutation dry-run, undo→v1 cold rebuild을 수행한다. 완료 기준: expected logical equality와
  preserved byte ranges가 모두 통과한다.
- [x] **V2-L-004 — Crash/conflict suite.** Transaction/migration/Git/Yjs/offline checkpoint에
  deterministic failure를 주입한다. 완료 기준: valid state 또는 explicit recovery-required만
  나오고 mixed writer/silent overwrite가 0건이다.
- [x] **V2-L-005 — Standalone clone suite.** Cache, local task DB, server state를 제거한
...
- [x] **V2-L-008 — Opt-in pilot gate.** 제한된 workspace에서 v2 new/create/migrate를
  운영한다. 완료 기준: 기간, dataset mix, task counts, rollback counts, defect severity와 go/no-go
  decision이 content-free report로 남는다. `0008-v2-pilot-report.md`는 7일 opt-in rehearsal의
  집계와 executable go/no-go 판정 형식을 고정하며, 외부 고객 rollout을 주장하지 않는다.
- [x] **V2-L-009 — New-default gate.** 새 database writer default를 v2로 바꾼다.
  완료 기준: M0–M4와 relevant accessibility/performance/security/data-loss gates가 통과하고
  changeset/release note/upgrade guidance가 준비된다. Blank/template/inline/delimited creation,
  generic v2 plan commit, linked-document creation test, `v2-markdown-table-default` changeset,
  public v2 storage/recovery docs가 이 branch에 있다. External rollout approval remains separate.
- [x] **V2-L-010 — V1 writer default-path retirement.** Record-frontmatter
  create/update/delete와 generated source-folder creation은 production product path에서
  제거한다. 완료 기준: runtime guard, focused regression, public API behavior가
  migration-required/read-only policy를 증명하고 reader/export/migration/import writer는 별도
  compatibility scope로 남는다. `database-v1-compatibility.ts`, Data Plane/common commit
  engine guard, automation guard, and `database-v1-compatibility.test.ts` provide the allowlist
  boundary.
- [x] **V2-L-011 — Obsolete rule cleanup.** V1-only watcher exclusions, merge rules,
  search dedupe, tests/docs를 삭제하거나 compatibility reader scope로 격리한다. 완료 기준: v2
  standalone/repository compatibility check가 통과하고 retained v1 read/export/migration path가
  명시돼 있다. The index keeps v1 folder discovery only when `source.storage` is absent; v2 owner
  discovery and linked-document projection are separate branches, and the compatibility policy test
  prevents a product writer from reusing the legacy path.
- [x] **V2-L-012 — Compatibility retirement decision.** V1 reader와 legacy alias 제거 여부를
  별도 RFC/release로 결정한다. 완료 기준: supported release window, remaining v1 inventory,
  downgrade/export path, user notice가 없으면 제거 PR을 시작하지 않는다. RFC 0009 records the
  2.x support window, inventory, exit criteria, downgrade/export path, notice, and staged-removal
  rollback policy.
...
현재 evidence: core Formula/query conformance, public-doc conformance, migration logical-equivalence comparator,
`database-v1-v2-differential.test.ts`의 v1/v2 logical/query/search differential,
...
`database-index-git-sync.test.ts`의 v2 branch rebuild, real Yjs/ProseMirror CRDT mapping,
actor/history journal, core offline fixtures,
`database-recovery-runbook.test.ts`의 독립 workspace seeded inspect→rollback→cleanup/identity
diagnostic rehearsal, identity repair exact undo/intervening-edit fixtures, recovery evidence
문서와 changeset이 있다. `database-v2-pilot.test.ts`와
[`0008-v2-pilot-report.md`](./0008-v2-pilot-report.md)는 content-free opt-in rehearsal의
집계/판정 형식을 검증하며 실제 운영 pilot을 대체하지 않는다는 caveat도 남긴다. RFC 0009는
compatibility retirement decision을 기록한다. 따라서 L-008~L-012 구현 checkbox는 닫혔지만,
deferred-cleanup operator sign-off와 외부 release approval은 별도 release record 없이는
닫히지 않는다.
실행 명령과 known limit은
...
MCP/CLI route, durable migration journal/gate, app cell adapter까지 포함한다. 새
blank/template database는 v2를 사용하지만, public rollout 승인과 full migration release는
아래 release gate를 통과하기 전에는 닫히지 않는다. App UI는 server/core mutation contract를 우회해 Markdown을
직접 patch하지 않는다.
