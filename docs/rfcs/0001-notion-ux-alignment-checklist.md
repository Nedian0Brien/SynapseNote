...
6. **Human vocabulary first.** The standard canvas, inline block, table, peek,
   and page paths use Notion-facing nouns (`page`, `property`, `view`, and
   `linked view`). Internal terms such as `canonical`, `schema`, `index`,
   `query`, `mutation`, `projection`, and stable IDs stay in Advanced,
   agent, diagnostic, or recovery surfaces unless they are necessary to
   explain an actionable user-facing error.
...
| Inline block   | Fresh `New database`/`Linked view of database`/`Inline database` inserts use a catalog/source/view picker; raw references remain advanced for existing MDX, and the block renders shared records with visible tabs and full-page handoff. | The core inline/linked journey is now unified for creation, editing, conversion, removal, and saved-view lifecycle/default handoffs; the complete visual state matrix remains open. |
...
| Views          | Saved views now render as visible tabs with `+`; tab actions route Filters/Settings directly, lifecycle changes through the reviewed manager boundary, Rename through the reviewed rename dialog, and Make/Clear default through the reviewed default boundary. `Manage views` remains available for full administration. | The primary view switch and inline lifecycle/default controls are closer to Notion; visual and complete state-matrix evidence remain open. |
...
- [x] **UX-011** Complete a user-facing database vocabulary and progressive-
      disclosure audit. The normal canvas/inline/peek/page journey must say
      `page` rather than `record` where Notion users see the object, keep
      `database`, `view`, `property`, and `linked view` as the primary nouns,
      and move `canonical`, `schema`, `index`, `query`, `mutation`,
      `projection`, machine IDs, and similar implementation language behind
      Advanced, agent, diagnostic, or recovery disclosures. Update visible
      errors, loading/recovery copy, menus, and accessible names together;
      add focused DOM and screen-reader copy assertions without changing the
      internal stable-ID/API vocabulary. Evidence (2026-07-23): page-aware
      terminology is propagated through table, inline linked layouts, peek,
      page chrome, loading/error/recovery states, menus, and accessible names;
      stable implementation IDs remain in advanced disclosures. Focused DOM
...
- [ ] **UX-1104** E2E row creation, typed cell editing, reload persistence, and
      ...
      undo/redo, reload persistence, and stable record-route opening in system
      Chrome (`8fc8bb0f`). Typed non-Title property editors and the remaining
      primary-view matrix remain open.
...
- [x] **UX-1107** E2E row → peek → full page → return to the same view context.
      ...
      plus the original `All tasks` tab remain selected in system Chrome
      (`8fc8bb0f`).
...
- [ ] **UX-1110** Automated accessibility plus manual keyboard/screen-reader
  ...
  `DB-A11Y-02`). The first seeds a canonical record through the same plan/commit
  API as the agent path and audits the real Table workspace opened from
  `Databases`; the second creates an inline database through the document slash
  flow, saves a title-based row, and audits the ready linked Table. Both run
  axe-core with WCAG 2.1 tags against only the database surface and block
