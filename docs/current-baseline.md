...
이 문서는 SynapseNote의 다음 작업 기준점을 고정하기 위한 기록이다. 제품 판단의 기준은 현재 운영 중인 AppFlowy-derived SynapseNote UI다.
...
- 확인 경로: `/`, `/login`, `/home`
- 확인 결과: 세 경로 모두 `200`
- 운영 방식: `.worktrees/appflowy-cloud`의 Docker compose에서 `appflowy_web` 서비스만 SynapseNote 로컬 이미지로 교체
...
- 기준 이미지: `synapsenote/appflowy-web:local`
- 배포 이미지 digest: `sha256:15a589d4da28ff7238bbe388dcf6dd8f498491c8a9d6c222923dbf6fedbc15d6`
- 컨테이너 재생성 시각: `2026-06-30T11:32:14Z`

## Cloud 기준

- 작업 위치: `.worktrees/appflowy-cloud/`
- 운영 서비스: `appflowy_cloud`, `appflowy_web`, `appflowy_worker`, `gotrue`, `postgres`, `redis`, `minio`, `nginx`
- 현재 문서 저장 원본: AppFlowy Cloud의 collab/Postgres/MinIO 계층
...
docker compose ps
docker compose logs --tail=50 appflowy_web
