...
SynapseNote는 앞으로 파일 기반 노트 저장소를 우선한다. 단, 제품 기준 UI는 현재 배포된 AppFlowy-derived SynapseNote UI다.

## 기준 원칙

1. 현재 배포 UI와 라우팅, 브랜딩, 사용자 흐름을 기준점으로 유지한다.
2. 문서 원본 저장소는 AppFlowy collab 계층에서 Markdown vault로 옮긴다.
...
- 현재 운영 원본: AppFlowy Cloud collab/Postgres/MinIO
...
- AppFlowy-derived UI에서 문서 목록, 문서 열기, 문서 저장에 필요한 최소 API 계약을 정의한다.
- Markdown vault API를 현재 UI가 읽을 수 있는 형태로 맞춘다.
- 기존 AppFlowy collab 저장소와 새 Markdown vault 저장소를 동시에 지우거나 병합하지 않는다.
