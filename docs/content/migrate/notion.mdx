---
title: From Notion
description: How to export a Notion workspace to Markdown and open it in SynapseNote, what carries over, and where the two differ.
icon: LuFileInput
---

This guide will cover exporting your Notion workspace to markdown files so that you can access them from SynapseNote. SynapseNote and Notion differ in a few key ways, so a Notion export needs some cleanup — this guide covers what carries over and what to fix.

## Install SynapseNote

### macOS (Apple Silicon)

Download the latest version of the SynapseNote desktop app.
...
<Callout type="info">
  ...
  from **File → Set up SynapseNote integrations…**, then open a new terminal
  window. If it's still missing, add `$HOME/.ok/bin` to your `PATH` manually.
</Callout>

Every transform is **idempotent**, so re-running is safe. See the tables below for a full list of differences between the Notion export and what SynapseNote supports.
...
## Open the export in SynapseNote

### macOS Desktop app

Open SynapseNote, choose **Open folder on disk**, select the unzipped export folder, and proceed through the **Setup** screen with defaults selected.
...
| **Callouts** | Exported as raw `<aside>` HTML, so they don't pick up SynapseNote's callout styling. | ✅ Converted to native `> [!note]` callouts. |
