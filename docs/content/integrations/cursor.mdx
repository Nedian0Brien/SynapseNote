---
title: Cursor
description: Use SynapseNote with Cursor.
icon: custom/Cursor
---

SynapseNote plugs into Cursor through MCP plus an installed skill that teaches the agent how to use the tools.
...
Cursor prompts you to approve a new MCP server when you next open the project. Approve `synapsenote`. Until you do, the agent can't see the tools.
...
Cursor uses a two-step handoff: SynapseNote spawns `cursor <projectDir>` to open the workspace first, then fires the `cursor://` URL with a short prompt asking Cursor to open the target in SynapseNote's web preview. The agent then grounds the rest of the conversation through MCP. See [Editor → Open with AI](/docs/features/editor#open-with-ai) for the full UX.
...
If it still doesn't see the tool after approval, restart Cursor so it picks up the new MCP entry. If it's still missing and `ok init` reported your config was `left unchanged`, SynapseNote couldn't safely edit `~/.cursor/mcp.json` for the reason shown in parentheses (unreadable, too large, or a duplicate server block) — fix the config and re-run `ok init`.
