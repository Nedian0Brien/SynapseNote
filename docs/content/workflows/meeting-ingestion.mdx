...
icon: LuMic
description: "Get meeting transcripts into your knowledge base as markdown, where they are searchable, linkable, and answerable. SynapseNote does not record meetings. You bring a recorder, OK holds the result."
---

SynapseNote does not capture audio. It is the knowledge layer: the moment a meeting lands as a markdown doc it is searchable, linkable, and answerable like everything else in your knowledge base. Capture is bring-your-own, so any recorder that speaks MCP can feed OK.
...
1. Add your recorder's MCP server to your **editor's** config, next to `synapsenote`. OK has no MCP client of its own, so this is editor config, not OK config.
...
The [OpenClaw](../integrations/openclaw.mdx) gateway has a built-in scheduler. Register both MCP servers with it, `synapsenote` via `ok init` and your recorder's MCP per the vendor's install guide, then create one cron job that wakes an agent to run the on-demand flow for you:
