...
SynapseNote scaffolds and edits that Markdown. Garry Tan's [`gbrain`](https://github.com/garrytan/gbrain), if you install it, can import/sync the same vault and add its DB-backed retrieval, graph extraction, embedding, and automation. OK does **not** replace `gbrain`; it gives GBrain-style Markdown brains a human cockpit for review, correction, attribution, and Git-visible edits.
...
| Layer | SynapseNote | Garry Tan's `gbrain` |
| --- | --- | --- |
...
Seeding needs an initialized project — run [`ok init`](../get-started/quickstart.mdx) first. Pick **Personal CRM** in the starter-pack picker, or run:
...
SynapseNote does not record meetings. Bring any recorder that speaks MCP, and each meeting lands in `meetings/` as markdown, ready for the loop below. Ask for them on demand, or have them sync automatically.

See [Ingest meetings](./meeting-ingestion.mdx) for the recorder list, the `source` and `source_meeting_id` addressing rule, and the scheduled and event-driven options.
