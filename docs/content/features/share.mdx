...
title: Share links
icon: LuShare2
description: Copy a link to any doc or folder and send it to a teammate. SynapseNote handles the GitHub round-trip, the deep link, and the branch on the other end.
---

SynapseNote has a Share button in the editor toolbar. Click it and the app copies a `https://synapse.lawdigest.kr/d/…` URL to your clipboard. Anyone you send it to lands on a splash page that knows the project and target, and a single click opens it in their copy of SynapseNote.
...
A share link points at GitHub, but your local edits only reach GitHub through auto-sync (off by default) or a manual push. When you click Share, SynapseNote checks the shared doc or folder against its last-known state on GitHub and, if the link won't yet show what you're looking at, adds a non-blocking warning row to the popover:
