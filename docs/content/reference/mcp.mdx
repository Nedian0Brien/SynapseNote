...
title: MCP
icon: custom/MCP
description: Tools the SynapseNote MCP server exposes to AI agents.
---
...
MCP tool approval, database Review/Balanced/Autonomous modes, Agent Views, and delegation budgets govern operations that pass through SynapseNote's Data Plane. They are audit and safety controls, not a sandbox. An agent with unrestricted shell or direct filesystem access can read or modify canonical Markdown, manifests, and owner-only `.ok/local/` policy state without calling these tools. Strict enforcement requires running the agent in a sandbox that prevents direct canonical writes and allows them only through the Data Plane. Do not treat an autonomy mode or a denied MCP call as protection from a fully privileged local process.

Every other read tool (`search`, `links`, `history`, etc.) and all write tools route through the Hocuspocus server, which SynapseNote auto-starts on the first call that needs it, under the same `OK_MCP_AUTOSTART` gate. All tools take a `cwd` (an absolute path inside the target project) that selects which project the call lands on; a globally registered server requires it unless the client advertises exactly one root.

The surface is **29 tools**. The four write verbs — `write`, `edit`, `delete`, `move` — are native CRUD operations polymorphic over a target. `write`, `edit`, and `delete` nest per-target fields inside the address key (`write({ document: { path, content } })`) and take exactly one target per call: `document`, `folder`, `template`, or `skill` — plus `asset` for `write` and `delete`, and a `documents` batch on `write` that writes several docs in order, each entry carrying its own `summary`. `move` takes flat `from`/`to` paths and auto-detects a document, folder, or asset, with nested `template` and `skill` targets for those two.
...
| `current_document` | Read the editor's current document identity and selection context when a browser session is attached. |
| `data` | Discover, describe, read, find, query, aggregate, and build token-budgeted context packs from file-native databases using stable IDs and permission-scoped projections. `kind: "retrieve"` explicitly selects lexical, semantic, or hybrid search; semantic retrieval reuses the project-local opt-in embeddings configuration and reports model, privacy, freshness, permission exclusions, deterministic fusion contributions, and any lexical degradation. Use `requireSemantic: true` to fail instead of degrading. Use `kind: "template_runs"` with a stable database ID to retrieve bounded, content-free repeating-template success/retry/failure history. Saved-view queries return compact conditional-color rule cards and matched IDs only for the returned permission-scoped page; denied rule dependencies fail closed. |
| `data_button` | Plan a database/source header Button or exact-revision Button property, execute the reviewed composite plan, or inspect bounded content-free run history. Execution commits verified internal changes first and then resumes ordered idempotent external delivery under the same durable receipt. |
| `data_place_search` | Submit one bounded Place name/address query to the operator-configured geocoder. Requires literal per-call consent, sends no autocomplete keystrokes, returns provider attribution and canonical coordinates, and reports an offline fallback when no provider is configured. This outbound tool is never auto-approved. |
| `data_plan` | Create ephemeral desired-state drafts and immutable snapshot-bound database plans. View conditional-color filters and targets accept agent-friendly property keys and rule keys converge on stable IDs. `preview_property_conversion` classifies the complete type-matrix edge, reports every revision-bound invalid or lossy row, requires a separate `allowLossy: true` approval, and compiles schema plus value rewrites into one ordinary plan. This does not write canonical files. |
| `data_commit` | Commit one unchanged exact database plan through approval, write guards, verification, audit receipt, idempotency, and undo. |
| `data_comments` | Read, create, reply to, edit, resolve, and reopen page- or property-level database record threads. Begin with `action: "read"`; every mutation uses the returned `revision` as `expectedRevision`. Agent authorship comes from the MCP connection, and mentions use active stable person IDs. |
| `data_undo` | Preview conflicts and apply an approved database transaction undo. |
| `data_repair` | Preview and apply bounded canonical/index database repairs through the reviewed repair transaction path. |
| `data_task` | Read-only preview existing-folder onboarding with `preview_import` and manifest migration with `preview_migration`, then launch and inspect durable bulk/import/migration tasks with progress, cancellation, retry, and recovery metadata. Import start refuses incomplete or unresolved input; migration start re-previews every selected manifest and refuses unsupported, invalid, lossy, or missing migration paths before queueing. |
