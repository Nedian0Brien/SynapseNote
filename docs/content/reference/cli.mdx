...
title: CLI & web app
icon: LuTerminal
description: Install the ok CLI and run the SynapseNote web app in your browser — on Linux, Windows, or an Intel Mac.
---
...
<Callout type="info" title="Prerequisites">
  - **Node.js 24+.** SynapseNote requires [Node.js](https://nodejs.org) 24 or higher.
  - **`git`.** SynapseNote initializes a git repo for your project (used for the timeline and recovery features). On macOS it comes with the Xcode Command Line Tools (`xcode-select --install`); on Linux or Windows, install it from [git-scm.com](https://git-scm.com) or your package manager.
</Callout>


## Install

Install the SynapseNote CLI globally so you can run `ok` from anywhere:
...
Run `ok init` in any folder to turn it into an SynapseNote project. It scaffolds a `.ok/` directory and registers the SynapseNote MCP server with the AI editors it detects on your machine (Claude Code, Claude Desktop, Cursor, Codex, OpenCode, OpenClaw, Pi, Antigravity, LM Studio, Hermes). It only ever adds its own entry — your other settings, comments, and formatting are left untouched.
...
`ok init` asks two questions: where to register the MCP server (user-level, project-level, or both) and whether to share the SynapseNote config files with your team (committed alongside your content) or keep them local to this machine.
...
For a complete list of every file SynapseNote creates or changes — during install, at runtime, and in the desktop app — see [What SynapseNote writes to your system](/docs/reference/what-synapsenote-writes).
...
You can pull repositories directly from GitHub and open them with SynapseNote:

```bash
ok clone <owner/repo>
```

`ok clone` accepts a full `https://github.com/...` URL or the `owner/repo` shorthand. It clones the repository, initializes SynapseNote if it isn't already, and starts the local server — open the printed Editor URL in your browser.
...
| `ok ps` | List every running SynapseNote server on your machine |
...
| `ok skills manage` | Control whether SynapseNote adopts your editor skills into this project (`--on`, `--off`, `--status`; default off) |
...
`ok uninstall` removes SynapseNote from the whole machine — running servers, credentials, the PATH shim, its own editor MCP entries, skill bundles, app data, and `~/.ok`. Your markdown content stays, and skills you authored under `~/.ok/skills` are kept unless you pass `--purge-content`. It never deletes the CLI binary itself; it ends by printing the removal command (`npm uninstall -g @nedian0brien/synapsenote` for the install above).

Both commands print their plan and ask before acting; `--dry-run` previews without changing anything.

On the macOS desktop app, **App menu → Uninstall SynapseNote…** covers both steps from one flow: it optionally deinitializes your projects, removes the global footprint, and then guides you through dragging the app to the Trash.
