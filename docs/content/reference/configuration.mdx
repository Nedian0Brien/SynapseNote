...
SynapseNote reads YAML config from three places:
...
| `agents.autoApproveOkTools` | boolean | `true` | user | Auto-approve SynapseNote's own MCP tools (and the `ok open` command on Claude) for agents launched from the built-in [docked terminal](/docs/features/editor#terminal), so reading and writing the knowledge base runs without a per-call approval prompt. High-impact or outbound tools (`delete`, `move`, `share_link`, `install`, `data_commit`, `data_undo`, `data_repair`, `data_task`, `data_place_search`) still prompt; other shell commands and non-SynapseNote file edits are untouched. Per-machine preference; toggle from **Settings → Terminal**. Best-effort per agent — Claude uses an allow/deny list, Codex uses its per-server `approve` mode (and only when its SynapseNote entry is already configured). |
...
| `OK_MCP_AUTOSTART` | Set to `0` to stop MCP tool calls from auto-starting the project's SynapseNote server; tools error until you run `ok start` |
...
| Variable | Sets |
| --- | --- |
| `SYNAPSENOTE_GITHUB_CLIENT_ID` | Override the GitHub OAuth App client ID |
| `SYNAPSENOTE_DATABASE_SANDBOX_MODE` | Set to `data-plane-only` for a database-governed agent deployment. The built-in Codex launcher forces a read-only filesystem regardless of renderer input, and both stdio and HTTP MCP servers expose only read/discovery and `data*` Data Plane tools. Start the desktop/server and `ok mcp` from the same environment. External agents must also run with a read-only project mount and no direct access to the project HTTP port; their only write-capable channel should be the restricted MCP process. |
...
| `OK_BUG_REPORT_INTAKE_URL` | Base URL of the bug-report intake the desktop app's **Help → Report a Bug…** upload posts to (e.g. `https://synapse.lawdigest.kr`). Must be `https:` — plain `http:` is accepted only for loopback hosts (local testing). Unset (the default), Send makes no network request — it opens a prefilled email draft to support@lawdigest.kr that you send yourself. |
...
| `OK_DATABASE_PLACE_GEOCODER_URL` | Explicit self-hosted or contracted Nominatim-compatible Place search URL. Must use HTTPS (or loopback for development); the public community Nominatim endpoint and credential-bearing URLs are rejected. Unset keeps Place search offline. |
| `OK_DATABASE_PLACE_GEOCODER_ATTRIBUTION` | Provider attribution returned with every Place search result. Required together with the URL and User Agent; partial configuration remains offline. |
| `OK_DATABASE_PLACE_GEOCODER_USER_AGENT` | Identifying User Agent for the configured Place provider. Required together with the URL and attribution. |
