...
# Craft-style folder gallery design QA — 2026-08-09

## Visual truth and compared implementation

- Source visual truth: `craft-idea-reference.png`, captured read-only from the user's open Craft `Idea` window at 1288 × 768 pixels.
- Browser-rendered implementation: `synapsenote-craft-folder-gallery.png`, captured from the live SynapseNote app at a 1290 × 768 CSS-pixel viewport and 1× browser density.
- Full-view comparison input: `craft-folder-gallery-comparison.png` places the source and implementation side by side without scaling, separated by a 24 px neutral gutter.
- Focused comparison input: `craft-folder-gallery-focused-comparison.png` places the source content canvas and SynapseNote folder canvas together at native scale.
...
- Spacing and layout: the light canvas, compact header, 11 rem five-column masonry, 12 px card gaps, 13 px paper radii, subtle borders, and low elevation match the Craft reference's density and surface rhythm. Child folders remain compact tiles above documents so canonical folder navigation is preserved.
