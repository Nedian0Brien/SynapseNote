...
7. **Current document:** A host-injected `<current_document>` block is authoritative for “what document is open?”, “내가 지금 보고 있는 문서 뭐야?”, and equivalent questions. Answer from it directly. If that block is absent, route to `current_document` FIRST. Never substitute Chronicle, screen/activity history, browser inspection, filesystem recency, or a guess; those tools are only for an explicit screen/history request. If neither source is available, say the live selection cannot be determined.

## Tool index — 24 tools (router; the MCP tool descriptions carry each tool's full contract)

- **Reads** — `exec` (primary; read-only `cat`/`ls`/`grep`/`find`/`head`/`tail`/`wc`/`sort`/`uniq`/`cut` + frontmatter/backlink/history enrichment; one command or one pipe, not a shell), `search` (ranked BM25 + recency), `history` (versions for a doc), `links` (`kind: backlinks|forward|dead|orphans|hubs|suggest`, or an array for a one-call audit), `skills` (find + read skills: omit `name` to LIST every skill across Project + Global, pass `name` to READ one — addressed by `name`+`scope`, NEVER by path), `config` (resolved config), `palette` (markdown-native authoring forms + `html preview` starters + theme tokens; `palette({ components })` for JSX schemas), `preview_url` (browser preview URL on demand), `share_link` (GitHub-substrate share URL; read-only, errors when no GitHub remote), `current_document` (the focused visible SynapseNote document; first fallback when the host did not inject `<current_document>`).
- **Data** — `search` returns stable database IDs; `data` reads catalog/describe/find/query/pack; `data_plan` → review → `data_commit` handles exact writes; `data_undo`, `data_repair`, and `data_task` cover recovery and durable work. Never write database-owned Markdown directly. Read the full contracts in [`references/tool-index.md`](references/tool-index.md).
- **Writes** — native CRUD (`write`, `edit`, `delete`, `move`) plus `install`, `checkpoint`, and `restore_version`; pass exactly one typed target and preserve the preview envelope. Skill authoring routes to `synapsenote-write-skill`. Full target shapes and approval rules are in [`references/tool-index.md`](references/tool-index.md).
- **Conflicts / workflow** — use `conflicts` / `resolve_conflict` and `workflow` (`ingest|research|consolidate|discover`); see [`references/conflict-resolution.md`](references/conflict-resolution.md) and [`references/workflow-guides.md`](references/workflow-guides.md).
