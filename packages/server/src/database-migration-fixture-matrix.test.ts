...
function record(
  ...
  path = 'items/one.md',
): { databaseId: string; sourceId: string; path: string; markdown: string } {
  return {
  ...
  test('covers generated blank, existing folder, inline/full-page, relation, codecs, lifecycle, and line endings', () => {
    const cases = [
      {
        ...
        input: {
          ...
          owners: [{ sourceId: 'ds_items', path: 'items.md', blockId: 'dbb_items' }],
          records: [],
        },
        expected: 'ready',
      },
      ...
      },
      {
        ...
        input: {
          definition: definition(),
          owners: [{ sourceId: 'ds_items', path: 'notes/inline.md', blockId: 'dbb_inline' }],
          records: [record('text: Inline')],
        },
        expected: 'ready',
      },
      {
        ...
        input: {
          definition: definition(),
          owners: [{ sourceId: 'ds_items', path: 'items/full-page.md', blockId: 'dbb_full' }],
          records: [record('text: Full page')],
        },
        expected: 'ready',
      },
      {
        ...
        input: {
          ...
          records: [
            record('related: [rec_target]', 'items/one.md'),
            {
              databaseId: 'db_matrix',
              sourceId: 'ds_targets',
      ...
      },
      {
        ...
        input: {
          definition: definition(),
          owners: [{ sourceId: 'ds_items', path: 'items.md', blockId: 'dbb_items' }],
          records: [record('number: not-a-number')],
        },
        expected: 'blocked',
      },
      {
        ...
        input: {
          ...
          records: [
            {
              ...record('text: lifecycle'),
              markdown: record('text: lifecycle').markdown.replace(
                '  record_id: rec_one',
      ...
      },
      {
        ...
        input: {
          ...
          records: [
            {
              ...record('text: "한글 — unicode"'),
              markdown: record('text: "한글 — unicode"')
                .markdown.replaceAll('\n', '\r\n')
                .replace(/^/, '\uFEFF'),
            },
          ],
      ...
      },
      {
        ...
        input: {
          definition: definition(),
          owners: [{ sourceId: 'ds_items', path: 'items.md', blockId: 'dbb_items' }],
          records: [{ ...record(`text: ${'x'.repeat(70_000)}`) }],
        },
        expected: 'blocked',
      },
    ] as const;
    ...
    for (const [index, fixture] of cases.entries()) {
      ...
      if (fixture.expected === 'blocked')
        expect(outcomes[index]?.blockers.length, fixture.id).toBeGreaterThan(0);
    }
