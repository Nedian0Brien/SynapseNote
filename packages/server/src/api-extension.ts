...
import * as contentShowAllWalk from './content-show-all-walk.ts';
...
import { createManagedRenameAssetExecutors } from './managed-rename-asset-executor.ts';
import { createManagedRenameContentOperations } from './managed-rename-content.ts';
import { createManagedRenameCoordinator } from './managed-rename-coordinator.ts';
import { createManagedRenameDocumentExecutor } from './managed-rename-document-executor.ts';
import { createManagedRenameEnumeration } from './managed-rename-enumeration.ts';
import {
  getMetrics,
...
export function createApiExtension(options: ApiExtensionOptions): Extension {
  ...
  // Durable rename execution is deliberately assembled outside the HTTP facade.
  // The route below retains admission, response mapping, attribution, cache invalidation,
  // and summary concerns while these collaborators preserve the mutation ordering.
  const managedRenameRuntime = {
    contentDir,
    projectDir,
    contentFilter,
    hocuspocus,
    ...
    mutateFileIndex,
    runSerialized,
    reconcileDiskBeforeAgentWrite: (docName: string) =>
      reconcileDiskBeforeAgentWrite(hocuspocus, docName, contentDir),
    renameFolderIndexEntries,
    deleteReconciledBase,
    setReconciledBase,
    registerWrite,
    contentHash,
    composeAndWriteRawBody: (document: unknown, markdown: string) =>
      composeAndWriteRawBody(document as never, markdown, 'managed-rename', false),
    managedRenameOrigin: MANAGED_RENAME_ORIGIN,
    isDocInConflict,
    withSpan,
    withSpanSync,
    recordContributor,
  ...
  const managedRenameContent = createManagedRenameContentOperations(managedRenameRuntime);
  const managedRenameEnumeration = createManagedRenameEnumeration(managedRenameRuntime);
  const captureAndCloseDocuments = managedRenameContent.captureAndCloseDocuments;
  const listManagedDocNamesUnderFolderFromDisk =
    managedRenameEnumeration.listManagedDocNamesUnderFolderFromDisk;
  const resolveExtensionlessAssetPath = managedRenameEnumeration.resolveExtensionlessAssetPath;
  const { executeAssetRename, executeDocumentToFileRename } = createManagedRenameAssetExecutors(
    managedRenameRuntime,
    managedRenameContent,
  );
  const executeManagedDocumentRename = createManagedRenameDocumentExecutor(
    managedRenameRuntime,
    managedRenameContent,
    managedRenameEnumeration,
  );
    ...
    async (req, res) => {
      try {
        ...
        if (showAll && contentFilter && showAllWantsNdjson(req)) {
          ...
          try {
            const maxEntries = contentShowAllWalk.getShowAllMaxEntries();
            const generator = contentShowAllWalk.streamShowAllEntries({
              contentDir,
              contentFilter,
              dirFilter: dir,
              maxEntries,
        ...
        if (showAll && contentFilter) {
          ...
          if (!entry) {
            ...
            const promise = (async (): Promise<ShowAllWalkResult> => {
              ...
              const maxEntries = contentShowAllWalk.getShowAllMaxEntries();
              const { truncated } = await contentShowAllWalk.walkContentDirForShowAll({
                contentDir,
                contentFilter,
                dirFilter: dir,
                documents,
        ...
        for (const [docName, entry] of allFiles) {
          ...
          if (passesDir && !assetPaths.has(docName)) {
            const assetExt = contentShowAllWalk.synthesizeShowAllAssetExt(docName);
            documents.push({
              kind: 'file',
              docName,
              path: docName,
          ...
          for (const alias of entry.aliases) {
            ...
            const targetRelPath = toPosix(relative(contentDir, entry.canonicalPath));
            const assetExt = contentShowAllWalk.synthesizeShowAllAssetExt(alias);
            documents.push({
              kind: 'file',
              docName: alias,
              path: alias,
          ...
          for (const [docName, dEntry] of allFiles) {
            projectChild(docName, (aliasName) => {
              ...
              if (dEntry.kind === 'markdown') {
                ...
                const assetExt = contentShowAllWalk.synthesizeShowAllAssetExt(aliasName);
                documents.push({
                  kind: 'file',
                  docName: aliasName,
                  path: aliasName,
  ...
  async function buildWorkspaceSearchDocumentsFromIndex(): Promise<{
    ...
    const maxFiles = contentShowAllWalk.getSearchMaxEntries();
    ...
    const databaseSearch = buildDatabaseSearchDocuments(contentShowAllWalk.getSearchMaxEntries());
    const documents = [
      ...pages,
