 ...
 * (`SynapseNote (<name>)` → `<name>`). The default install (userData basename
 ...
 */
const DEFAULT_USERDATA_NAMES = new Set(['SynapseNote', 'SynapseNote', 'Electron']);

/** Strip the `SynapseNote (x)` / `SynapseNote (x)` dev wrapper down to `x`. */
const DEV_WRAPPER = /^(?:SynapseNote|SynapseNote) \((.+)\)$/;
...
/** Suffix the app name with the instance label, e.g. `SynapseNote` → `SynapseNote (work)`. */
