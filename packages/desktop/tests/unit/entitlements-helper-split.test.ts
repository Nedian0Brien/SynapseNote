 ...
 * notably `com.apple.developer.associated-domains`) must never be granted to
 ...
 * .mac.plist`) keeps the file-access entitlements that helpers do not need.
 * Universal Links are intentionally disabled until SynapseNote has its own
...
describe('macOS helper-process entitlements (Tahoe AMFI compliance)', () => {
  ...
  test('electron-builder.yml entitlements (main-app) points at the main plist (not the helper plist)', () => {
    ...
    // points at a plist with no main-process file access entitlements.
  ...
  test('main-app plist still carries file-access entitlements (regression guard)', () => {
    // Pin every file-access entitlement the main app needs that helpers must NOT have.
    ...
    const keys = extractKeys(content);
    for (const required of MAIN_ONLY_REQUIRED_KEYS) {
      expect(keys, `${required} must appear in main-app entitlements`).toContain(required);
    }
  });

  test('main-app plist omits associated-domains until a SynapseNote profile is configured', () => {
    const content = readFileSync(mainPlist, 'utf8');
    expect(extractKeys(content)).not.toContain('com.apple.developer.associated-domains');
  });
