...
test.describe('document-native database browser journeys', () => {
  test('normal New-page Database choice lands on an editable table and creates a page row', async ({
    page,
  }) => {
    await page.goto('/');
    await page.getByRole('button', { name: 'New file', exact: true }).click();
    await expect(page.getByRole('dialog', { name: 'New file' })).toBeVisible({ timeout: 5_000 });
    await page.getByTestId('new-item-dialog-new-database').click();
    ...
    await expect(page.getByRole('gridcell', { name: 'New-page first record' })).toBeVisible({
      timeout: 15_000,
    });
  });
  ...
  test('slash Inline database stays in the document and hands off to the canonical page', async ({
    ...
    await expect(inline.getByRole('gridcell', { name: 'Inline first record' })).toBeVisible({
      timeout: 15_000,
    });
  ...
  test('linked view preserves shared rows through peek, full page, and return', async ({
    ...
    const savedViewButtons = picker.getByRole('button');
