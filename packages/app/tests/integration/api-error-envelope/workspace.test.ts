...
import { ProblemDetailsSchema, WorkspaceSuccessSchema } from '@nedian0brien/synapsenote-core';
...
describe('workspace envelope (RFC 9457)', () => {
  test('happy path emits flat success body with application/json', async () => {
    ...
    const parsed = WorkspaceSuccessSchema.safeParse(body);
    expect(parsed.success).toBe(true);
    if (parsed.success) {
      expect(parsed.data.contentDir.length).toBeGreaterThan(0);
    }
