...
import { createSavedViewProjectionDraft } from './database-saved-view-settings-draft';
import {
  conditionalColorSummary,
  move,
  moveVisibleProperty,
} from './database-saved-view-settings-utils';
...
export function DatabaseSavedViewSettingsDialog({
  ...
  const projectionDraft = createSavedViewProjectionDraft(view, source);
  const [propertyOrder, setPropertyOrder] = useState(() => projectionDraft.propertyOrder);
  const [visiblePropertyIds, setVisiblePropertyIds] = useState(
    () => projectionDraft.visiblePropertyIds,
  );
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-4xl">
        ...
        <DialogBody className="space-y-5">
            ...
            >
              ...
              <p className="mt-1 text-muted-foreground text-xs">
                <Trans>
                  ...
                  options save together. Filters stay on the active view's Filters action and use
                  the same canonical view boundary.
                </Trans>
              </p>
            </section>
          ...
          <DatabaseSavedViewSettingsLayoutPanel>
            {view.layout.type === 'table' ? (
              <section
            ...
            {view.layout.type === 'timeline' ? (
              <section className="space-y-3" aria-label="Saved Timeline display settings">
                ...
                <div className="grid gap-3 sm:grid-cols-3">
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      <SelectItem
                        value="separate"
                        disabled={
                          source.properties.filter((property) => property.type === 'date').length <
                          2
                        }
                      >
                  ...
                  <Select
                    value={timelineStartPropertyId}
                    onValueChange={setTimelineStartPropertyId}
                  >
            ...
            {view.layout.type === 'map' ? (
              <section className="space-y-3" aria-label="Saved Map display settings">
                ...
                <p className="text-muted-foreground text-xs">
                  The local map makes no network requests. External tiles are available only when
                  the selected Place property explicitly permits external maps.
                </p>
                <div className="flex flex-wrap gap-4 text-sm">
                  {[
