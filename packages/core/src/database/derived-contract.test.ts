    ...
    sources: [
      {
        ...
        properties: [
          ...
          {
            id: 'prop_double', key: 'double', name: 'Double', type: 'formula', source: 'prop("score") * 2',
            ast: { language: 'synapse-formula-1', version: 1, resultType: 'number', expression: { type: 'binary', operator: 'multiply', left: { type: 'property', propertyId: 'prop_score' }, right: { type: 'literal', valueType: 'number', value: 2 } } },
          },
        ],
      },
    ],
...
describe('database derived contract', () => {
  ...
  test('surfaces formula source/AST drift before commit', () => {
    ...
    expect(contract.diagnostics.some((diagnostic) => diagnostic.code === 'formula_ast_mismatch')).toBe(true);
  });
  ...
  test('surfaces cross-property cycles with an exact dependency path', () => {
    ...
    formula.ast = { language: 'synapse-formula-1', version: 1, resultType: 'number', expression: { type: 'binary', operator: 'add', left: { type: 'property', propertyId: 'prop_double' }, right: { type: 'literal', valueType: 'number', value: 1 } } };
    const contract = compileDatabaseDerivedContract(value);
    expect(contract.diagnostics.some((diagnostic) => diagnostic.code === 'dependency_cycle')).toBe(true);
    expect(contract.diagnostics.find((diagnostic) => diagnostic.code === 'dependency_cycle')?.dependencyPath).toEqual(['prop_double', 'prop_double']);
  });
});
