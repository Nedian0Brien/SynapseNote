...
function removeLegacyDatabaseMetadata(markdown: string): string {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    while (
      cursor < lines.length &&
      (/^[ \t]+/.test(lines[cursor] ?? '') || (lines[cursor] ?? '').trim() === '')
    ) {
      const nestedLine = lines[cursor] ?? '';
      const ownedKey =
        /^\s{2}(?:database_id|source_id|record_id|created_at|last_edited_at|created_by|last_edited_by|archived_at|page_layout_override):\s*/.test(
          nestedLine,
        );
      if (ownedKey) {
        cursor += 1;
  ...
  const nextFrontmatter =
    frontmatter.slice(0, bodyOffset) + nextBody + frontmatter.slice(bodyOffset + body.length);
...
function propertyCellValue(
  ...
  if (property.type === 'title')
    return { ok: true, value: link(record.path, titleOverride ?? String(value ?? '')) };
  if (property.type === 'select' || property.type === 'status') {
    ...
    return option
      ...
      : {
          ok: false,
          message: `Property "${property.key}" has unknown option ID "${String(value)}"`,
        };
  }
  if (property.type === 'multi_select') {
    if (!Array.isArray(value))
      return { ok: false, message: `Property "${property.key}" is not an option array` };
  ...
  if (property.type === 'person') {
    if (!Array.isArray(value))
      return { ok: false, message: `Property "${property.key}" is not a person array` };
  ...
  if (property.type === 'relation') {
    ...
    if (
      !links.every((candidate): candidate is DatabaseMarkdownDocumentLink => candidate !== null)
    ) {
      return {
        ok: false,
        message: `Property "${property.key}" contains an unresolved relation target`,
      };
    }
  ...
  for (const propertyId of source.storage?.storedPropertyIds ?? []) {
    ...
    if (!property || DERIVED_PROPERTY_TYPES.has(property.type)) {
      return {
        ok: false,
        propertyId,
        message: 'Derived or unknown property cannot be stored in v2',
      };
    }
    ...
    const encoded = encodeDatabaseMarkdownCell(
      property.type as Parameters<typeof encodeDatabaseMarkdownCell>[0],
      projected.value,
    );
...
function ownerMarkdown(
  ...
  const headers =
    source.storage?.storedPropertyIds.map((propertyId) => {
      const name =
...
export function planDatabaseMarkdownV2Migration(input: {
  ...
  if (input.definition.version !== 1) {
    blockers.push({
      ...
      message: 'Only a v1 manifest can be migrated to v2',
    });
    return {
      status: 'blocked',
      definition: null,
  ...
  for (const owner of input.owners) {
    if (!safeOwnerPath(owner.path))
      blockers.push({
        code: 'unsafe_owner_path',
        sourceId: owner.sourceId,
        path: owner.path,
        message: `Owner path "${owner.path}" is unsafe`,
      });
    if (ownerPaths.has(owner.path))
      blockers.push({
        ...
        sourceId: owner.sourceId,
        path: owner.path,
        message: `Owner path "${owner.path}" is used more than once`,
      });
  ...
  for (const record of input.records) {
    ...
    if (!source) {
      blockers.push({
        ...
        path: record.path,
        message: `Source "${record.sourceId}" is not defined by the manifest`,
      });
      continue;
    }
    ...
    if (recordsById.has(recordId)) {
      blockers.push({
        ...
        sourceId: source.id,
        path: record.path,
        message: `Record ID "${recordId}" is duplicated`,
      });
      continue;
    }
    ...
    if (
      selectedTitle &&
      selectedTitle !== resolveDatabaseDocumentTitle(linkedMarkdown, record.path).value
    ) {
  ...
  for (const source of input.definition.sources) {
    ...
    if (!owner) {
      blockers.push({
        code: 'owner_path_collision',
        sourceId: source.id,
        message: `Source "${source.id}" has no selected owner path`,
      });
      continue;
    }
    ...
    for (const recordInput of input.records
      ...
      const materializedRecord = [...materialized.values()].find(
        (candidate) => candidate.path === recordInput.path,
      );
      ...
      const lastEditedTime = source.properties.find(
        (property) => property.type === 'last_edited_time',
      );
      ...
      const lastEditedAt = lastEditedTime
        ? materializedRecord.values[lastEditedTime.id]
        : undefined;
    ...
    if (
      new TextEncoder().encode(renderedOwner).byteLength >
