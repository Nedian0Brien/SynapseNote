...
export function planDatabaseMarkdownIdentityRepair(input: {
  ...
  const parsedOwners: Array<{
    document: DatabaseMarkdownIdentityRepairDocument;
    owner: ParsedDatabaseMarkdownOwner;
  }> = [];
  for (const document of input.owners) {
    ...
    if (ownerMatches(parsed.owner, input.databaseId, input.sourceId))
      parsedOwners.push({ document, owner: parsed.owner });
  }
  if (parsedOwners.length > 1) {
    for (const owner of parsedOwners) {
      issues.push({
        ...
        message: `Multiple owner tables claim source "${input.sourceId}"`,
      });
    }
  }
  ...
  for (const document of input.documents) {
    ...
    if (!parsed.ok) {
      ...
      if (
        proposed &&
        (parsed.code === 'missing_document_id' || parsed.code === 'missing_frontmatter')
      ) {
        ...
        if (ensured.ok) {
          actions.push({
            ...
            path: document.path,
            documentId: proposed,
            beforeSha256: digest(document.markdown),
            afterSha256: digest(ensured.markdown),
            afterMarkdown: ensured.markdown,
          });
  ...
  for (const [documentId, paths] of identities) {
    ...
    for (const path of paths)
      issues.push({
        ...
        path,
        message: `Document ID "${documentId}" is shared by multiple documents`,
      });
  }

  const owner = parsedOwners.length === 1 ? parsedOwners[0] : null;
  if (owner) {
    const aliasReplacements: Array<{ rowIndex: number; text: string }> = [];
    ...
    for (const row of owner.owner.rows) {
      ...
      if (!link) {
        issues.push({
          ...
          message: 'Title cell does not contain a valid document wikilink',
        });
        continue;
      }
      ...
      if (!documentPath) {
        issues.push({
          ...
          path: owner.document.path,
          rowIndex: row.rowIndex,
          message: `Document link "${link.target}" cannot be resolved`,
        });
        continue;
      }
      ...
      if (previous !== undefined)
        issues.push({
          ...
          path: owner.document.path,
          rowIndex: row.rowIndex,
          relatedPath: documentPath,
          message: `Rows ${previous} and ${row.rowIndex} point to the same document`,
        });
      ...
      const documentTitle = document
        ? resolveDatabaseDocumentTitle(document.markdown, document.path).value
        : null;
      if (document && documentTitle && link.alias !== documentTitle) {
        const replacement = encodeDatabaseMarkdownCell('title', {
          kind: 'wikilink',
          target: pathWithoutExtension(document.path),
          alias: documentTitle,
        });
        if (replacement.ok) {
          aliasReplacements.push({ rowIndex: row.rowIndex, text: replacement.text });
          issues.push({
            code: 'stale_alias',
            path: owner.document.path,
            rowIndex: row.rowIndex,
            relatedPath: document.path,
            message: `Title alias does not match the linked document title "${documentTitle}"`,
          });
        }
      }
    }
  ...
  const planWithoutHash = {
    ...
    committable: issues.every((issue) => issue.code === 'stale_alias') && parsedOwners.length === 1,
    issues,
    actions,
  };
