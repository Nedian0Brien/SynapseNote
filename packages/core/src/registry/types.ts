...
interface JsxComponentMetaBase {
  ...
  /**
   * Interaction semantics for the editor NodeView host.
   *
   * Most self-closing components are atomic: clicking their rendered body
   * selects the JSX node and the wrapper can be dragged directly. Interactive
   * leaf surfaces opt out of body selection and use the editor's explicit
   * block drag grip. Keeping this in the descriptor prevents the generic JSX
   * host from having to know component names.
   */
  interaction?: {
