...
const handleAwarenessUpdate = (ctx: SyncContext, message: collab.IAwarenessUpdate): void => {
    if (!ctx.awareness) {
        console.log(`No awareness instance found in SyncContext for objectId ${ctx.doc.guid}`);
    } else {
...
const handleUpdate = (ctx: SyncContext, message: collab.IUpdate): void => {
    ...
    console.log(`applied update to doc ${doc.guid}`);
    ...
    if (doc.store.pendingStructs || doc.store.pendingDs) {
        console.log(`Doc ${doc.guid} has missing dependencies. Sending sync request...`);
        emit({
            collabMessage: {
                objectId: doc.guid,
                collabType: ctx.collabType,
...
export const initSync = (ctx: SyncContext) => {
    ...
    console.log(`Initializing sync for objectId ${doc.guid} with collabType ${collabType}`);
