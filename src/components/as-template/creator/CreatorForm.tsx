...
>(({ defaultValues, onSubmit }, ref) => {
  ...
  const { watch, control, handleSubmit } = useForm<TemplateCreatorFormValues>({
    defaultValues,
  });
  ...
  return (
    <form
      className={'flex flex-col gap-4 overflow-hidden py-5'}
      ...
      <Controller
        ...
        render={({ field, fieldState }) => (
          <FormControl className={'flex flex-col gap-4'} error={!!fieldState.error}>
            <FormLabel error={!!fieldState.error} required className={'text-md flex items-center text-text-secondary'}>
